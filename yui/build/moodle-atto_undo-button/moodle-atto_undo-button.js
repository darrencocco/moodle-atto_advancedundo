...
// along with Moodle.  If not, see <http://www.gnu.org/licenses/>.
    ...
    Y.namespace('M.atto_undo').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
        ...
        _filterMouseEvent: function(e) {
            ...
            // Clicking the paragraph style button gives an event, however clicking a paragraph style option doesn't give a click event, just a 'focus' (like every other action).
            // This also behaves weirdly in that initially, all 'focus' event.type have a value of 'focus'. After the first time an option is clicked from the paragraph
            // style menu, all subsequent event.type have a value of 'focusoutside' !!

            var mouseEventState = {isMouseClick:false, isMouseClickInTextEditor:false, isMouseClickOnEditorButton:false, clickedEditorButtonClassname:''};
        ...
        _changeListener: function(e) {
            ...
            if (!this._redoButtonIsActive()) {
                ...
                // Allowing any event through here as there is a possibility it could have caused a text edit (thereby triggering the redo button to be disabled)
                // Ideally would only allow events of a specific type through (eg a key press or mouse click), however the issue
                // is with the paragraph style selector which does not register a mouse button click, so we cannot tell when it has been triggered!!!!
